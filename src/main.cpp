...
const uint8_t numPaddingBytes = HID_TX_SIZE - (sizeof(uint16_t) * numPots) - (sizeof(uint8_t) * numSwitchBytes);
...
static_assert(sizeof(report) == HID_TX_SIZE, "HIDPacket must be exactly 64 bytes");
